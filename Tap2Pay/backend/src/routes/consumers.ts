...
// Express 4 does not route async rejections to the error middleware. Without
// this wrapper a rejected query becomes an unhandledRejection, which index.ts
...
// Bug #14 fix (2026-09-02): previous SQL selected five columns that exist in no
// migration (reward_type, stamps_per_visit, redeem_threshold, lifetime_points,
...
router.post('/p2p-token', validate(p2pTokenSchema), asyncHandler(async (req: Request, res: Response) => {
  const consumerId = req.consumer!.sub
  ...
  try {
    ...
    if (p2pToken) {
      ...
      if (parsed.amountCents !== null && parsed.amountCents !== amountCents) {
        return res.status(400).json({ error: `Amount mismatch: payee requested KSh ${(parsed.amountCents / 100).toFixed(2)}` })
      }
    } else {
    ...
    logger.info('P2P transaction created', { txnId, p2pTxnId, payerConsumerId, payeeConsumerId, source, kesAmountCents })

    // ─── Fire STK Push to payer ───────────────────────────────────────────────
    const callbackUrl = `${process.env.DARAJA_CALLBACK_BASE_URL}/api/v1/mpesa-callback`
    const amountKsh = Math.ceil(kesAmountCents / 100)
    const description = payeeDisplayName ? `Send to ${payeeDisplayName}`.slice(0, 13) : 'P2P Transfer'
    ...
    if (!stkResult.success) {
      ...
      await db.query(
        `UPDATE p2p_transactions SET status = 'FAILED' WHERE id = $1`,
        [p2pTxnId]
      )
