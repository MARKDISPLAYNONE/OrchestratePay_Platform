...
describe('AdminFleetPage NFC colour coding', () => {
  it('applies green colour class when NFC is available', async () => {
    ...
    expect(screen.getByText('OK').className).toMatch(/text-emerald-400/)
  })
...
describe('AdminFleetPage — missing response keys', () => {
  it('shows empty fleet when getFleet response has no devices key', async () => {
    mockGetFleet.mockResolvedValue({})
    mockGetFleetAlerts.mockResolvedValue({})
    render(<AdminFleetPage />)
    await waitFor(() =>
      expect(screen.getByText(/0 registered terminal/i)).toBeInTheDocument()
    )
  })
})

describe('AdminFleetPage — unknown printer status', () => {
  it('falls back to raw printer_status value for unknown status codes', async () => {
    const deviceUnknownPrinter = { ...deviceHealthy, printer_status: 99 }
    mockGetFleet.mockResolvedValue({ devices: [deviceUnknownPrinter] })
    mockGetFleetAlerts.mockResolvedValue({ alerts: [] })
    render(<AdminFleetPage />)
    await waitFor(() =>
