...
describe('LoyaltyPage — missing balances key', () => {
  it('shows empty state when response has no balances key', async () => {
    mockGetLoyalty.mockResolvedValue({})
    render(<LoyaltyPage />)
    await waitFor(() =>
      expect(screen.getByText(/no loyalty cards yet/i)).toBeInTheDocument()
...
describe('LoyaltyPage — null redeem_threshold', () => {
  it('falls back to 1000 threshold when redeem_threshold is null', async () => {
    const balance = { ...pointsBalance, redeem_threshold: null, points_balance: 500 }
    mockGetLoyalty.mockResolvedValue({ balances: [balance] })
    render(<LoyaltyPage />)
    await waitFor(() =>
