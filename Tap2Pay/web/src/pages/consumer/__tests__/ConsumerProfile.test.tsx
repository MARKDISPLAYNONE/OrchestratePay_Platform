...
describe('ProfilePage — setTimeout callback coverage', () => {
  it('reverts "Saved ✓" button after 2.5s timer', async () => {
    mockGetProfile.mockResolvedValue(profileFull)
    mockUpdateProfile.mockResolvedValue({})
    render(<ProfilePage />)
    await waitFor(() => screen.getByRole('button', { name: /save changes/i }))
...
describe('ProfilePage — null display_name', () => {
  it('initialises display name as empty string when display_name is null', async () => {
    mockGetProfile.mockResolvedValue({ ...profileFull, display_name: null })
    render(<ProfilePage />)
    await waitFor(() => screen.getByLabelText(/display name/i))
...
describe('ProfilePage — null phone and email', () => {
  it('does not render Phone or Email rows when both are null', async () => {
    mockGetProfile.mockResolvedValue({ phone: null, email: null, display_name: 'Alice', sms_opt_in: false })
    render(<ProfilePage />)
    await waitFor(() => screen.getByLabelText(/display name/i))
