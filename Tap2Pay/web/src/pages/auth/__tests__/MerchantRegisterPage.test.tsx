...
describe('MerchantRegisterPage success', () => {
  ...
  it('shows a back-to-sign-in button in the success state', async () => {
    ...
    await fillAndSubmit()
    await waitFor(() =>
      expect(screen.getByRole('button', { name: /sign in to complete kyc/i })).toBeInTheDocument()
    )
  })
  ...
  it('navigates to /auth/login when back-to-sign-in is clicked', async () => {
    mockRegister.mockResolvedValue({ merchantId: 'mid-1', status: 'pending' })
    await fillAndSubmit()
    await waitFor(() => screen.getByRole('button', { name: /sign in to complete kyc/i }))
    await userEvent.click(screen.getByRole('button', { name: /sign in to complete kyc/i }))
