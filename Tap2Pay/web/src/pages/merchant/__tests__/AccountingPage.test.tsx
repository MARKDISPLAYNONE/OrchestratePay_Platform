...
describe('AccountingPage — missing integrations/postings keys (line 26)', () => {
  it('shows empty state when getIntegrations and getPostings responses have no keys', async () => {
    ...
    // No "Connected" badge → empty integrations
    expect(screen.queryByText('Connected')).not.toBeInTheDocument()
  })
})

describe('AccountingPage — Cancel button after Connect expand (line 63)', () => {
  beforeEach(() => {
    mockGetIntegrations.mockResolvedValue({ integrations: [] })
    mockGetPostings.mockResolvedValue({ postings: [] })
  })

  it('collapses token input when Cancel is clicked (isExpanded ? null : p.id TRUE branch)', async () => {
    render(<AccountingPage />)
    await waitFor(() => screen.getAllByRole('button', { name: /^connect$/i }))
    // First click: expand first platform (isExpanded=false → p.id)
    const [firstConnect] = screen.getAllByRole('button', { name: /^connect$/i })
    await userEvent.click(firstConnect)
    await waitFor(() => screen.getByPlaceholderText(/paste access token/i))
    // Second click: collapse (isExpanded=true → null)
    await userEvent.click(screen.getByRole('button', { name: /^cancel$/i }))
    await waitFor(() =>
      expect(screen.queryByPlaceholderText(/paste access token/i)).not.toBeInTheDocument()
    )
  })
})
