...
| **Web App** | Vite + React 19 + React Router v6 | `Tap2Pay/web/` |
...
└── Tap2Pay/
    ...
    │       ├── components/         GlassCard, Badge, KpiCard, layout shells
    │       └── lib/                Typed API client, JWT auth helpers
...
ngrok http 3000
# Copy the https URL → set DARAJA_CALLBACK_BASE_URL in backend/.env
```

### Option C — Step-by-step (no Docker)
...
cd Tap2Pay/backend
cp .env.example .env
# Edit .env: DATABASE_URL=postgresql://orchestratepay:devpassword@localhost:5432/orchestratepay
npm install
npm run migrate
...
**Terminal 3 — M-Pesa callbacks** (payment testing only)

```bash
ngrok http 3000
# Paste the https URL into backend/.env as DARAJA_CALLBACK_BASE_URL, restart backend
```
...
```bash
# Backend (71 suites, ~1,291 assertions)
cd Tap2Pay/backend
npm test
npm run test:coverage
npm run lint

# Web (23 suites, 452 assertions)
cd Tap2Pay/web
npm run test:unit
...
These issues **will cause a failed deployment** if not fixed before applying the K8s manifests.

### 1 — Env var name mismatch in K8s

`infra/k8s/backend/deployment.yaml` injects `DARAJA_CALLBACK_URL` but the backend requires `DARAJA_CALLBACK_BASE_URL`.

**Fix:** Rename the env var key in `deployment.yaml`.

### 2 — Missing secrets in K8s

`ADMIN_SECRET` and `NFC_SIGNING_SECRET` are absent from `infra/k8s/secrets.template.yaml` and `deployment.yaml`.

**Fix:** Add both to `secrets.template.yaml` and reference them in `deployment.yaml`.

---

## Pre-launch Checklist

- [x] Distributed Redlock on reconciliation job — `util/distributed-lock.ts`
- [x] CI/CD pipeline — `.github/workflows/ci.yml`
- [x] Kubernetes manifests — `infra/k8s/` (see P0 gaps above)
- [x] JWT refresh token rotation — `merchant_refresh_tokens` table
- [x] Sentry integration — `util/sentry.ts`
- [x] `/readiness` probe — checks PostgreSQL + Redis, returns 503 on failure
- [x] Account lockout — Redis-based per-email (5 attempts → 15 min)
- [x] Migration versioning — `src/db/migrations/` with versioned runner
- [x] Admin endpoint audit logging — `writeAuditLog` on fleet config + FX force-refresh
- [x] TLS certificate pins — ISRG Root X1/X2 in `network_security_config.xml`
